...
tracker = DeepSort(
    ...
    nn_budget=100,
    embedder="mobilenet", # Efficient for real-time
    half=True # Speed boost if using CUDA
)
frame_idx = 0
...
while vs.running():
    ...
    # 3. DEEPSORT TRACKING (This replaces your matching and manual aging logic)
    tracks = tracker.update_tracks(detections, frame=frame)

    for track in tracks:
        # Only process tracks that are confirmed by the tracker
        ...
        # Calculate foot point for mapping
        fx, fy = (x1 + x2) // 2, y2

        # --- Visuals on Main Video ---
        draw_3d_bbox(vis, x1, y1, x2, y2)
        cv2.putText(vis, f"ID {track_id}", (x1, max(30, y1-12)), 
                    cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 0, 255), 3)
        
        # --- 4. HOMOGRAPHY & COURT MAPPING ---
        # Map the foot point to the 2D court coordinates
        mapped = cv2.perspectiveTransform(np.array([[[float(fx), float(fy)]]], dtype=np.float32), H)[0][0]
        cx, cy = mapped
        
        # Check if the player is within court boundaries
        if LINE_MARGIN < cx < 10 - LINE_MARGIN and LINE_MARGIN < cy < 6.5 - LINE_MARGIN:
            mx, my = court_to_pixel(cx, cy)
            
            # Draw on the 2D Mat
            cv2.circle(mat, (mx, my), 8, (255, 0, 0), -1)
            cv2.putText(mat, f"{track_id}", (mx + 6, my - 6), 
