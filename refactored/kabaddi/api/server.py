...
- Live MJPEG streams of the processing pipeline
- State snapshots and event logs
- Archived video playback
- Event details and clips
"""
...
def attach_queues(frame_queue, state_queue, input_queue=None, log_queue=None):
    """
    Attach processing queues to the app state.
    Called by the main processing loop to inject live data.
    """
    app.state.frame_queue = frame_queue
    app.state.state_queue = state_queue
...
def set_run_id(run_id: str):
    """Set a unique run identifier for this processing session."""
...
def _combined_slices(frame_width: int) -> dict[str, tuple[int, int, int, int]]:
    """
    Compute crop rectangles from the combined frame.
...
def _crop_combined_frame(bgr_frame, kind: str):
    """
    Crop `vis`/`mat`/`graph` from the combined dashboard frame.
    """
...
def _videos_dir() -> Path:
    # Expect `Videos/` in the repository root (2 levels up from this file).
...
def _latest_video(glob_pattern: str) -> Optional[Path]:
    ...
    def _is_probably_playable_mp4(path: Path) -> bool:
        """
        Heuristic filter to avoid non-playable MP4 files.
        """
        try:
            st = path.stat()
        except OSError:
...
if __name__ == "__main__":
    ...
    #   python -m kabaddi.api.server
    # Recommended (equivalent):
    #   uvicorn kabaddi.api.server:app --host 0.0.0.0 --port 8000
    import uvicorn

    port = int(os.environ.get("PORT", "8000"))
    uvicorn.run("kabaddi.api.server:app", host="0.0.0.0", port=port, log_level="info")
