...
from kabaddi.core import (
    VideoStream, apply_optical_flow, run_yolo_detection,
    update_tracks, add_new_tracks, render_gallery
)
...
from kabaddi.utils import select_line_coordinates
from kabaddi.api import attach_queues, set_run_id
...
ACTIVE_LINES, ACTIVE_CAMERA = select_line_coordinates(VIDEO_PATH, lines, lines2)
...
# ======================================================
# LIVE STREAMING API (FASTAPI) - OPTIONAL
# ======================================================
LIVE_FRAME_QUEUE = queue.Queue(maxsize=2)
LIVE_STATE_QUEUE = queue.Queue(maxsize=2)
LIVE_INPUT_QUEUE = queue.Queue(maxsize=2)
LIVE_LOG_QUEUE = queue.Queue(maxsize=50)

# Generate unique run ID for this processing session
RUN_ID = hashlib.md5(VIDEO_PATH.encode("utf-8")).hexdigest()[:8]

def start_api_server():
    """Start FastAPI server in a background thread for live streaming."""
    import uvicorn
    from kabaddi.api import app

    # Attach live queues to app state
    attach_queues(LIVE_FRAME_QUEUE, LIVE_STATE_QUEUE, LIVE_INPUT_QUEUE, LIVE_LOG_QUEUE)
    set_run_id(RUN_ID)

    uvicorn.run(app, host="0.0.0.0", port=8000, log_level="error")

# Start API server in daemon thread (optional - can be disabled)
ENABLE_API_SERVER = os.getenv("ENABLE_API_SERVER", "true").lower() in ("true", "1", "yes")
if ENABLE_API_SERVER:
    api_thread = threading.Thread(target=start_api_server, daemon=True)
    api_thread.start()
    print(f"[API] Live streaming server started at http://0.0.0.0:8000 (Run ID: {RUN_ID})")
...
while vs.running():
    ...
    # Push frames to live queues for API streaming (non-blocking)
    if ENABLE_API_SERVER:
        try:
            LIVE_FRAME_QUEUE.put_nowait(combined_frame.copy())
        except queue.Full:
            pass
        try:
            LIVE_INPUT_QUEUE.put_nowait(frame.copy())
        except queue.Full:
            pass
        try:
            state_snapshot = {
                ...
                "current_raid_points": CURRENT_RAID_POINTS,
                "raider_id": RAIDER_ID,
                "num_tracks": len(GALLERY),
                "confirmed_events_count": len(CONFIRMED_EVENT_LOG),
            }
            LIVE_STATE_QUEUE.put_nowait(state_snapshot)
        except queue.Full:
            pass
